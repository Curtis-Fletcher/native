'use strict'
const debug = require('debug')('vue-json-schema:parser')
    ...
    case 'object':
      ...
      keys.sort(
        (a, b) => {
          ...
          const maxOrd = keys.length + 1
          const aOrd = (sp[a].attrs ? sp[a].attrs.order : maxOrd) || maxOrd
          const bOrd = (sp[b].attrs ? sp[b].attrs.order : maxOrd) || maxOrd
          debug('%s %d | %s %d', a, aOrd, b, bOrd)
